...
impl Polkit {
    pub fn new(connection: Connection, namespace: &'static str) -> Self {
        Self {
            connection,
            namespace,
        }
    ...
    pub async fn authorize(&self, sender: Option<&str>, action: &str) -> bool {
        ...
        let action_id = format!("{}.{}", self.namespace, action);
...
async fn get_connection_unix_user(connection: &Connection, name: &str) -> anyhow::Result<u32> {
    let proxy = Proxy::new(
        connection,
